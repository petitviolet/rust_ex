use crate::token::{ Loc, Annotation, Token };

#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub enum AstKind {
  Number(u64),
  UniOperator { operator: UniOperator, tree: Box<Ast> },
  BinOperator { operator: BinOperator, lhs: Box<Ast>, rhs: Box<Ast> },
}
...
impl Ast {
  ...
  fn uni_operator(op: UniOperator, tree: Ast, loc: Loc) -> Self {
    Self::new(AstKind::UniOperator { operator: op, tree: Box::new(tree) }, loc)
  }

  fn bin_operator(op: BinOperator, lhs: Ast, rhs: Ast, loc: Loc) -> Self {
    Self::new(AstKind::BinOperator { operator: op, lhs: Box::new(lhs), rhs: Box::new(rhs) }, loc)
  }
}
