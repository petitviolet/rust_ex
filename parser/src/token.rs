...
mod lex {
  ...
  fn consume_byte(input_bytes: &[u8], position: usize, expected: u8) -> LexResult<u8> { 
    if input_bytes.len() <= position {
      ...
      Err(LexError::invalid_char(input_bytes[position] as char, Loc::new(position, position)))
    } else {
  ...
  fn consume_bytes_until(input_bytes: &[u8], mut position: usize, mut f: impl FnMut(u8) -> bool) -> usize {
  ...
  pub fn number(input_bytes: &[u8], position: usize) -> LexResult<Token> { 
    ...
    let n = std::str::from_utf8(&input_bytes[start..end]).unwrap().parse().unwrap(); 
...
pub fn lex(input: &str) -> Result<Vec<Token>, LexError> {
  ...
  while position < input_bytes.len() {
    match input_bytes[position] { 
      ...
      b => return Err(LexError::invalid_char(b as char, Loc::new(position, position + 1))),
    }
  }
